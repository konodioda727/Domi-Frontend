  ...
  return (
    <>
      <View className='review-searchbar'>
        <SearchbarPrefix searchTypes={searchTypeItems} onChooseItem={handleOnChooseItem}></SearchbarPrefix>
        <Input className='review-searchbar-input' value={inputValue}></Input>
...
const SearchbarPrefix: React.FC<SearchbarPrefixProps> = (props) => {
  const [showPrefixChooseList, setShowPrefixChooseList] = useState<boolean>(false);
    ...
    <>
      <View className='review-searchbar-prefix' onClick={handlePrefixClick}>
        <View className='review-search-wrap'>
          ...
          <Image src='https://s2.loli.net/2024/02/08/JZs53xBENFMQyVp.png' className='review-prefix-icon'></Image>
        </View>
        {showPrefixChooseList && <View className='review-choose-list'>
          {searchTypes.map((item) => <View onClick={()=>handleChooseItem(item)} key={autoKey.next().value as number} className='review-choose-list-item'>{item.disp_text}</View>)}
        </View>}
        {showPrefixChooseList && <View onClick={handleCloseList} className='review-hover'></View>}
      </View>
    </>
  )
}
  ...
  return (
    <>
      <View className={`review-searchbar-suffix ${className}`} {...restProps}>
        <Image className='review-searchbar-suffix-icon' src='https://s2.loli.net/2024/02/08/eoYdxk45GwNO7Hr.png'></Image>
      </View>
    </>
  )
}
